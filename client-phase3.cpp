            ...
            for(int i=0;i<=max_fd;i++){
                if(FD_ISSET(i,&fd_active)){
                    if(i == socket_server){
                        ...
                        string yes = "1", no = "0"; int send_file,send_size;
                        if(is_present){
                        	int sent = send(i,yes.c_str(),strlen(yes.c_str()),0 );
                        	string filenamepath= argv2+name;
				...
				int* fsize = &size;
				send_size= send(i,(char*)fsize,sizeof(int),0);
  				send_file = send(i, file_buffer, size, 0);
...
void connecting_to_neighbours( int n, string data,int unique_id, int files_num, vector<string> files_data,char* argv2){
    ...
    for(int i=0;i<files_num;i++){
		...
		ifstream file(argv2+d+name, ifstream::binary);
		MD5_CTX md5Context;
		MD5_Init(&md5Context);
		char buff[1024 * 16];
		while (file.good()) {
		...
		for (const auto &byte: result)
        	...
        	cout  <<"Found " << files_data[i] << " at " <<  *min_element(fileExistence[i].begin(), fileExistence[i].end()) << " with MD5 "<<sl<<" at depth 1";
        }
...
int main(int argc, char* argv[]){
    ...
    if(configfile.is_open()){
        ...
        thread th2(connecting_to_neighbours,n , data4, unique_id, files_num, files_data,argv[2]);
